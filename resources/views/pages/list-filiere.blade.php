...
<section
    ...
    <div class="w-full  bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6 md:p-8 overflow-x-auto">
        ...
        <form method="get" class="mb-6 flex justify-between items-center" action="{{ route('filiere.index') }}">

            <div class="flex flex-wrap items-center gap-4 my-4">

                {{-- Filtrer par nom --}}
                <input type="text" name="name" placeholder="Rechercher une filiere par nom..."
                    value="{{ request('name') }}"
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6 flex flex-col justify-between">
                <div>
                    ...
                    <p class="text-gray-700 dark:text-gray-300 mb-4">
                        @if(strlen($filiere->description) > 40)
                        {{ Str::limit($filiere->description, 40) }}
                ...
                <div class="flex space-x-2 mt-2">
                    <a href="{{ route('filiere.edit', $filiere->id) }}"
                        class="text-blue-600 hover:text-blue-900">Modifier</a>
                    <form method="post" action="{{ route('filiere.delete', $filiere->id) }}">
                        @csrf
                        @method('DELETE')
